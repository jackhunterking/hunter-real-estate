...
export async function GET(request: NextRequest) {
  ...
  if (!client) {
    destination.pathname = portalPublicPath(hostname, "/sign-in");
  ...
  const next = safePortalNext(hostname, request.nextUrl.searchParams.get("next"));
  ...
  destination.pathname = error ? portalPublicPath(hostname, "/sign-in") : next;
  destination.search = "";
