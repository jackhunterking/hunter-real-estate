 ...
 * an elegant generated placeholder (see lib/equity-market/present.ts).
 */
export type ImageSlot = {
  src?: string;
  /**
   * Supabase Storage reference. When present, the server read layer
   * (lib/equity-market/repository-server.ts) resolves `{bucket,path}` to a public
...
export type OfferingDocument = {
   ...
   * short-lived signed URL from /api/portal-offering-documents.
   */
 ...
 * write — use `riskText()` (lib/equity-market/present.ts) to read one.
 */
