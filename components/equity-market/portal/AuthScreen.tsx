      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col items-center px-5 py-10 sm:px-8 sm:py-14">
        <section className="w-full max-w-[460px] rounded-lg border border-[#d9dee2] bg-white px-6 py-8 shadow-[0_8px_24px_rgba(20,38,52,0.05)] sm:px-9 sm:py-9" aria-labelledby="auth-title">
          {awaitingConfirmation ? (
            <div className="flex flex-col items-center text-center">
              ...
              <div className="mt-7 w-full border-t border-[#e2e6e8] pt-6">
                <Link href={`${INVESTMENT_BASE_PATH}/sign-in`} className="text-sm font-semibold text-[#173b57] hover:underline">
                  {c.backToSignIn}
                </Link>
              </div>
            </div>
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <label className="block text-sm font-medium text-[#283640]">
              <span className="flex items-center justify-between gap-4">
                ...
                {mode === "sign-in" && (
                  <Link href={`${INVESTMENT_BASE_PATH}/forgot-password`} className="text-xs font-semibold text-[#173b57] hover:underline">
                    {c.forgotPassword}
                  </Link>
                )}
              </span>
            ...
            {accountMissing && (
              <Link
                href={`${INVESTMENT_BASE_PATH}/sign-up${continuationSearch}`}
          ...
          <div className="mt-7 border-t border-[#e2e6e8] pt-6 text-center">
            <Link href={`${INVESTMENT_BASE_PATH}/${mode === "sign-in" ? "sign-up" : "sign-in"}${continuationSearch}`} className="text-sm font-semibold text-[#173b57] hover:underline">
              {mode === "sign-in" ? c.switchToSignUp : c.switchToSignIn}
            </Link>
            {!configured && (
              <div className="mt-5 text-xs leading-5 text-[#6b7680]">
                <p>{c.notConfigured}</p>
                <Link href={`${INVESTMENT_BASE_PATH}/home`} className="mt-2 inline-block font-semibold text-[#173b57] hover:underline">
                  {c.preview}
                </Link>
              </div>
            )}
