...
import { alignFundPeriods, belowMinimum, inceptionYear, type AlignedPeriod } from "@/lib/equity-market/compare-funds";
import type { Lang, LocalizedText } from "@/lib/equity-market/types";
import { useLang } from "@/lib/i18n/LanguageProvider";
import { pick, tx } from "@/lib/i18n/localize";
import { INVESTMENT_BASE_PATH } from "@/lib/equity-market/investment-brand";
import { Panel, money } from "@/components/equity-market/portal/PortalUI";
import { DisclosureBar } from "@/components/equity-market/portal/DisclosureBar";
import { FundSelect, HeroCashFlow, PrimaryAmountField, ToolHeader } from "@/components/equity-market/portal/CompareUI";
...
export function CompareFunds({ funds }: { funds: FundComparable[] }) {
  ...
  function selectFund(side: "a" | "b", id: string) {
    ...
    posthog?.capture("em_fund_compare_fund_selected", {
      language: lang,
  ...
  function selectRow(key: string) {
    setRowKey(key);
    posthog?.capture("em_fund_compare_period_selected", { language: lang, period_key: key });
  }
