...
import {
  ...
  type FundComparable,
  type FundPeriod,
} from "@/lib/equity-market/compare-investments";
import { EquityMarketBrand, ParvisCoBrand } from "../EquityMarketBrand";
...
const SIGN_UP = `${INVESTMENT_BASE_PATH}/sign-up?path=investor`;
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[#e0e6ea] bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 sm:px-8">
        <EquityMarketBrand dark showCoBrand={false} />
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`${INVESTMENT_BASE_PATH}/sign-in`}
...
const COMPARE_TONES: { self: CompareTone; portal: CompareTone; markets: CompareTone }[] = [
  { self: "mixed", portal: "good", markets: "mixed" }, // what you own
  { self: "poor", portal: "good", markets: "mixed" }, // who does the work
  { self: "mixed", portal: "good", markets: "mixed" }, // where income comes from
  { self: "poor", portal: "good", markets: "good" }, // getting started
  { self: "poor", portal: "good", markets: "mixed" }, // how you get in
  { self: "poor", portal: "good", markets: "good" }, // debt in your name
  { self: "poor", portal: "good", markets: "mixed" }, // spreading the risk
  { self: "mixed", portal: "good", markets: "mixed" }, // day-to-day swings
  { self: "poor", portal: "good", markets: "good" }, // registered accounts
  { self: "poor", portal: "good", markets: "good" }, // getting out
];
...
export function HowItCompares({ c }: { c: LandingCopy }) {
  ...
  const tones = (i: number) =>
    COMPARE_TONES[i] ?? { self: "mixed" as const, portal: "good" as const, markets: "mixed" as const };
        ...
        <div className="overflow-hidden rounded-2xl border border-[#dbe1e5] bg-white">
          ...
          <div className="grid grid-cols-[1fr_1fr] border-b border-[#e2e8eb] sm:grid-cols-[0.8fr_1fr_1fr]">
            ...
            <div className="border-l border-t-2 border-l-[#e2e8eb] border-t-[#c5a34d] bg-[#f6f9fa] px-4 pb-3.5 pt-3">
              <span className="block text-[10px] font-bold uppercase tracking-[0.12em] text-[#a8873a]">{c.compare.portalBadge}</span>
              <span className="mt-0.5 block text-sm font-semibold text-[#0a2d46]">{cols.portal}</span>
            </div>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm leading-6 text-[#40515e] sm:border-l sm:border-[#e2e8eb] sm:bg-[#f6f9fa] sm:px-4 sm:py-4">
                    ...
                    <CompareCellBody text={row.portal} tone={t.portal} highlight />
                  </div>
                </div>
              );
            })}
