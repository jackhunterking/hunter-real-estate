...
import type { LearningResourceSummary } from "@/lib/equity-market/learning";
...
export function LearningLibrary({ resources }: { resources: LearningResourceSummary[] }) {
  ...
  const visible = useMemo(() => resources.filter((resource) => {
    ...
    return `${tx(resource.title, lang)} ${tx(resource.summary, lang)}`
  ...
  function recordSearch() {
    ...
    posthog?.capture("em_learning_search_used", { language: lang, query_length: normalized.length });
  }
      ...
      {visible.map((resource, index) => {
        ...
        return <article key={resource.id} className="group relative overflow-hidden rounded-lg border border-[#d8e0e4] bg-white shadow-[0_1px_2px_rgba(8,34,52,0.04)]">
          ...
          <div className="p-6 sm:p-7">
            ...
            <h2 className="mt-5 max-w-2xl font-serif text-2xl font-semibold leading-tight text-[#102c3f] sm:text-[1.75rem]">{tx(resource.title, lang)}</h2>
            <p className="mt-3 text-sm leading-6 text-[#5a6c77]">{tx(resource.summary, lang)}</p>
            ...
            <Link href={`${INVESTMENT_BASE_PATH}/resources/learning/${resource.slug}`} className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[#0a4b72] after:absolute after:inset-0">
