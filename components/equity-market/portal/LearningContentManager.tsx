...
import { type LearningAdminResource, type LearningResourceSource } from "@/lib/equity-market/learning";
...
export function LearningContentManager({ resources, backendConfigured }: { resources: LearningAdminResource[]; backendConfigured: boolean }) {
  ...
  async function save() {
    ...
    const url = draft.resourceId && draft.versionId
      ? `/api/portal-learning-resources/${draft.resourceId}/versions/${draft.versionId}`
      : "/api/portal-learning-resources";
  ...
  async function transition(action: "submit" | "request_changes" | "approve" | "publish" | "withdraw" | "new_version") {
    ...
    await request(`/api/portal-learning-resources/${draft.resourceId}/versions/${draft.versionId}/transition`, "POST", { action, note: reviewNote || undefined });
  }
