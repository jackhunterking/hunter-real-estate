...
import type { LearningResourceDetail } from "@/lib/equity-market/learning";
import { learningCategoryLabel } from "@/lib/equity-market/learning";
...
export function LearningGuide({ resource }: { resource: LearningResourceDetail }) {
  ...
  useEffect(() => {
    posthog?.capture("em_learning_guide_opened", { slug: resource.slug, version: resource.version, language: lang });
  }, [lang, posthog, resource.slug, resource.version]);

  return <article className="mx-auto max-w-5xl">
    <Link href={`${INVESTMENT_BASE_PATH}/resources/learning`} className="inline-flex items-center gap-2 text-sm font-semibold text-[#496779] hover:text-[#0a4b72]"><ArrowLeft className="size-4" />{c.back}</Link>
    ...
    <aside className="mt-7 rounded-lg border border-[#d8e0e4] bg-white p-6 sm:flex sm:items-center sm:justify-between sm:gap-6">
      ...
      <Link href={`${INVESTMENT_BASE_PATH}/resources/investor-readiness`} className="mt-5 inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-md bg-[#0a2d46] px-4 text-sm font-semibold text-white sm:mt-0">{c.open}<ArrowRight className="size-4" /></Link>
    </aside>
  </article>;
}
