...
import {
  ...
  type PortalUser,
  type PreviewPersona,
} from "@/lib/equity-market/portal-access";
import { calculateFundDistributionCommission } from "@/lib/equity-market/commissions";
...
export function PortalAccessProvider({
  ...
  useEffect(() => {
    ...
    try {
      const savedPersona = window.localStorage.getItem("em-preview-persona") as PreviewPersona | null;
  ...
  useEffect(() => {
    ...
    try {
      const stored = window.localStorage.getItem("em-account-view");
  ...
  function setPreviewPersona(persona: PreviewPersona) {
    ...
    try {
      window.localStorage.setItem("em-preview-persona", persona);
    } catch {
  ...
  function setAccountView(view: PortalAccountView) {
    ...
    try {
      window.localStorage.setItem("em-account-view", view);
    } catch {
