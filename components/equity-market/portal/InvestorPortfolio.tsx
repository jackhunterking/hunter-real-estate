...
import { buildMapProperties, formatCurrencyCad, formatMoneyCompact, primaryShareClass } from "@/lib/equity-market/present";
import { PositionCard, type IncomePeriod } from "./PositionCard";
import { latestPublished12mReturn, parseTargetMidpoint, portfolioMonthlyIncome, type MonthlyIncomeBreakdown } from "@/lib/equity-market/performance";
import { unitPriceOf, impliedShares } from "@/lib/equity-market/shares";
import { taxonomyLabel } from "@/lib/equity-market/taxonomies";
import { requestStage } from "@/lib/equity-market/investment-requests";
import { useLang } from "@/lib/i18n/LanguageProvider";
import { pick, tx } from "@/lib/i18n/localize";
import { cn } from "@/lib/utils";
import { FundMap } from "@/components/equity-market/map/FundMap";
import { OfferingSummaryCard, offeringBundleCardProps } from "@/components/equity-market/OfferingSummaryCard";
import { InfoPopover } from "@/components/equity-market/InfoPopover";
...
const PERIOD_KEY = "em-position-period";
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {opportunities.map((fund) =>
              renderFundCard(
                ...
                fund,
                <Link
                  href={`${INVESTMENT_BASE_PATH}/investments/${fund.slug}`}
      ...
      {opportunities.length === 0 && heldFunds.length === 0 && reviewFunds.length === 0 && (
        <Panel className="mt-8 grid min-h-[280px] place-items-center p-8 text-center">
          <div className="max-w-md">
            ...
            <Link href={`${INVESTMENT_BASE_PATH}/investments`} className="mt-6 inline-flex h-10 items-center rounded-md bg-[#0a2d46] px-4 text-sm font-semibold text-white">{c.viewOffering}</Link>
          </div>
        </Panel>
      )}
    </div>
