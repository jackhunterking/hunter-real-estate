...
export function OfferingContentManager({ offerings, backendConfigured }: { offerings: OfferingAdminRow[]; backendConfigured: boolean }) {
  ...
  async function save() {
    if (!bundle) return;
    const result = await request("/api/hnc-offerings", { bundle });
    if (!result) return;
  ...
  async function publish() {
    ...
    const result = await request("/api/hnc-offerings/publish", { offeringId });
  ...
  async function uploadAsset(file: File, folder: "media" | "docs", bucket: "offering-public" | "offering-private"): Promise<{ bucket: string; path: string } | null> {
    ...
    try {
      ...
      const res = await fetch("/api/hnc-offerings/upload-url", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bucket, path }) });
