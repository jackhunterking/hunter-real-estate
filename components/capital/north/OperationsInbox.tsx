...
import { Banknote, Building2, ClipboardCheck, FileSearch, Gauge, Search, ShieldCheck, UserCheck, X } from "lucide-react";
...
export type OperationsModule = "requests" | "professional" | "licences" | "firms" | "payments" | "fund-schedules" | "leads" | "audit";
...
const ICONS = { requests: ClipboardCheck, professional: UserCheck, licences: ShieldCheck, firms: Building2, payments: Banknote, "fund-schedules": Gauge, leads: FileSearch, audit: FileSearch };
const COPY = {
  en: { eyebrow: "Hunter North staff", title: "Operations", description: "One queue for investor requests, professional approvals, licence checks, firm records, payments, fund schedules, leads, and audit activity.", all: "All work", requests: "Investor requests", professional: "Professional applications", licences: "Licence checks", firms: "Firm records", payments: "Payments", fundSchedules: "Fund schedules", leads: "Legacy leads", audit: "Audit activity", search: "Search the queue", noItems: "No items match these filters.", details: "Review details", close: "Close drawer", approveFirm: "Approve firm record", approvePayment: "Approve payment", markPaid: "Mark paid", paymentRef: "Payment reference", required: "Required role", compliance: "Compliance", finance: "Finance", platform: "Platform admin" },
  tr: { eyebrow: "Hunter North ekibi", title: "Operasyonlar", description: "Yatırımcı talepleri, profesyonel onayları, lisans kontrolleri, firma kayıtları, ödemeler, fon programları, potansiyel müşteriler ve denetim faaliyeti için tek kuyruk.", all: "Tüm işler", requests: "Yatırımcı talepleri", professional: "Profesyonel başvuruları", licences: "Lisans kontrolleri", firms: "Firma kayıtları", payments: "Ödemeler", fundSchedules: "Fon programları", leads: "Eski potansiyel müşteriler", audit: "Denetim faaliyeti", search: "Kuyrukta ara", noItems: "Bu filtrelerle eşleşen kayıt yok.", details: "İnceleme ayrıntıları", close: "Paneli kapat", approveFirm: "Firma kaydını onayla", approvePayment: "Ödemeyi onayla", markPaid: "Ödendi olarak işaretle", paymentRef: "Ödeme referansı", required: "Gerekli rol", compliance: "Uyum", finance: "Finans", platform: "Platform yöneticisi" },
} as const;

export function OperationsInbox({ leads = [], initialModule = "all", initialQueue = [], offerings = [], backendConfigured = false }: { leads?: LeadRow[]; initialModule?: OperationsModule | "all"; initialQueue?: OperationsQueueItem[]; offerings?: { id: string; name: LocalizedText }[]; backendConfigured?: boolean }) {
  const { lang } = useLang();
  ...
  const allowedModules = useMemo(() => new Set<OperationsModule>([
    ...(compliance ? ["requests", "professional", "licences", "firms", "leads", "audit"] as OperationsModule[] : []),
  ...
  const labels: Record<OperationsModule, string> = { requests: c.requests, professional: c.professional, licences: c.licences, firms: c.firms, payments: c.payments, "fund-schedules": c.fundSchedules, leads: c.leads, audit: c.audit };
