...
import { Banknote, BookOpen, Building2, ClipboardCheck, FileSearch, Gauge, Landmark, Search, ShieldCheck, UserCheck, X } from "lucide-react";
...
export type OperationsModule = "requests" | "professional" | "licences" | "firms" | "payments" | "fund-schedules" | "content" | "offerings" | "leads" | "audit";
...
const ICONS = { requests: ClipboardCheck, professional: UserCheck, licences: ShieldCheck, firms: Building2, payments: Banknote, "fund-schedules": Gauge, content: BookOpen, offerings: Landmark, leads: FileSearch, audit: FileSearch };
const COPY = {
  en: { eyebrow: "Hunter & Hunter Investment Advisors staff", title: "Operations", description: "One workspace for investor requests, professional approvals, learning content, payments, and audit activity.", all: "All work", requests: "Investor requests", professional: "Professional applications", licences: "Licence checks", firms: "Firm records", payments: "Payments", fundSchedules: "Investment schedules", content: "Content", investments: "Investments", leads: "Legacy leads", audit: "Audit activity", search: "Search the queue", noItems: "No items match these filters.", details: "Review details", close: "Close drawer", approveFirm: "Approve firm record", approvePayment: "Approve payment", markPaid: "Mark paid", paymentRef: "Payment reference", required: "Required role", compliance: "Compliance", finance: "Finance", platform: "Platform admin" },
  tr: { eyebrow: "Hunter & Hunter Investment Advisors ekibi", title: "Operasyonlar", description: "Yatırımcı talepleri, profesyonel onayları, bilgi içeriği, ödemeler ve denetim faaliyeti için tek çalışma alanı.", all: "Tüm işler", requests: "Yatırımcı talepleri", professional: "Profesyonel başvuruları", licences: "Lisans kontrolleri", firms: "Firma kayıtları", payments: "Ödemeler", fundSchedules: "Yatırım programları", content: "İçerik", investments: "Yatırımlar", leads: "Eski potansiyel müşteriler", audit: "Denetim faaliyeti", search: "Kuyrukta ara", noItems: "Bu filtrelerle eşleşen kayıt yok.", details: "İnceleme ayrıntıları", close: "Paneli kapat", approveFirm: "Firma kaydını onayla", approvePayment: "Ödemeyi onayla", markPaid: "Ödendi olarak işaretle", paymentRef: "Ödeme referansı", required: "Gerekli rol", compliance: "Uyum", finance: "Finans", platform: "Platform yöneticisi" },
} as const;

export function OperationsInbox({ leads = [], initialModule = "all", initialQueue = [], offerings = [], offeringAdmin = [], learningResources = [], backendConfigured = false }: { leads?: LeadRow[]; initialModule?: OperationsModule | "all"; initialQueue?: OperationsQueueItem[]; offerings?: { id: string; name: LocalizedText }[]; offeringAdmin?: OfferingAdminRow[]; learningResources?: LearningAdminResource[]; backendConfigured?: boolean }) {
  const { lang } = useLang();
  ...
  const allowedModules = useMemo(() => new Set<OperationsModule>([
    ...(compliance ? ["requests", "professional", "licences", "firms", "content", "offerings", "leads", "audit"] as OperationsModule[] : []),
  ...
  const items = useMemo<QueueItem[]>(() => {
    ...
    if (allowedModules.has("payments")) dataset.commissions.forEach((payment) => queue.push({ id: `payment:${payment.id}`, module: "payments", title: payment.redactedReferralReference, summary: `${payment.amount} ${payment.currency} · ${payment.beneficiaryType}`, status: payment.status, date: payment.paidAt ?? payment.approvedAt ?? payment.fundedAt, details: { "Commission ID": payment.id, Investment: payment.offeringId, Beneficiary: payment.beneficiaryType, Amount: payment.amount, Currency: payment.currency, "Earning period": payment.earningPeriod, "Payment reference": payment.paymentReference } }));
  ...
  const labels: Record<OperationsModule, string> = { requests: c.requests, professional: c.professional, licences: c.licences, firms: c.firms, payments: c.payments, "fund-schedules": c.fundSchedules, content: c.content, offerings: c.investments, leads: c.leads, audit: c.audit };
