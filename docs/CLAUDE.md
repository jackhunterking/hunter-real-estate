...
- `components/` — `Nav`, `Footer`, `HomeHero`, `ServicesSection`, `ContactSection`, etc.
...
6. **The funds portal is not part of this site.** It lives in its own repo (github.com/jackhunterking/equity-market). Link out to it through `lib/portal-link.ts`; don't rebuild investment pages here.
