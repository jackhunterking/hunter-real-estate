-- Make the professional-partner intake work for any jurisdiction, not only
-- Türkiye/SPL. Adds an explicit jurisdiction field and drops SPL-specific wording.

alter table app.partner_applications
  add column if not exists jurisdiction text;

-- The parameter list changes (new p_jurisdiction), so replace the old signature
-- outright rather than leave a dangling overload.
drop function if exists app.submit_partner_application(
...
begin
  ...
  if length(trim(coalesce(p_registered_first_names, ''))) = 0
     ...
     or length(trim(coalesce(p_jurisdiction, ''))) = 0
  ...
  insert into app.audit_events (
    ...
    'Partner application submitted for independent firm and regulatory review.'
  );
